...
const props = withDefaults(defineProps<{
  title?: string
  accept?: string           // ex: ".pdf,.xlsx,.csv" — vide = tous types acceptés
  maxSizeMb?: number
  multiple?: boolean
  maxFiles?: number
}>(), {
...
const canImport = computed(() => importedFiles.value.length > 0)

const isMaxReached = computed(() =>
  props.multiple && importedFiles.value.length >= props.maxFiles
)
...
function handleImport(): void {
  ...
  emit('import', importedFiles.value.map(f => f.file))
}
...
<template>
  <div class="w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 flex flex-col gap-5 shadow-sm">
    ...
    >
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div class="flex justify-end gap-2">
      <button
        type="button"
        class="h-9 px-4 text-sm font-medium rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        @click="handleCancel"
      >
        Annuler
      </button>
      ...
      <button
        type="button"
        :disabled="!canImport"
        class="h-9 px-4 flex items-center gap-2 text-sm font-medium rounded-xl bg-blue-600 text-white hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:active:scale-100"
        @click="handleImport"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
        ...
        Importer
        <span v-if="importedFiles.length > 1" class="bg-blue-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-md leading-none">
