  ...
  <div class="p-6 max-w-2xl mx-auto">
    ...
    <div v-else class="space-y-4">
      <div
        ...
        :key="col.id"
        class="flex items-center gap-4 p-4 rounded-xl shadow border"
...
const fetchConfig = async (): Promise<void> => {
  loading.value = true;
  try {
    const res = await api.get<Column[]>('/kanban-config');
  ...
  try {
    await Promise.all(
      columns.value.map((col) =>
        api.put<{ success: boolean }, SaveConfigPayload>(`/kanban-config/${col.id}`, {
          color: col.color,
