...
export default class SupplierService {
    private readonly subEndPoint ='Supplier'
     private readonly endPointPrefix = `/Management/${this.subEndPoint}`
    async getAllIds(): Promise<number[]> {
        try {
            const endpoint = `query { ${this.subEndPoint} { id } }`;
            const response = await glpiApi.graphql<{ suppliers: { id: number | string }[]}>(endpoint);
            if (response.suppliers) {
                return response.suppliers.map(supplier => Number(supplier.id));
            }
